...
document.getElementById("keyComboBox").addEventListener("change", () => {
  if (localStorage.getItem(":j35mc:_boxChanged") != "") {
    ...
    document.getElementById("keyComboBox").value = localStorage.getItem(
      ":j35mc:_keyComboBoxB4",
    );
    return;
  }
  ...
  if (document.getElementById("pageTag").value === "*") {
    document.getElementById("feed_box").value = localStorage.getItem(
      ":j35mc:" +
        document.getElementById("localStoreKey").value,
    );
  } else {
...
document.getElementById("xxh32sum").addEventListener("click", (evt) => {
  // let copyText = evt;
  // copyText.select();
  // copyText.setSelectionRange(0, 99999); /*For mobile devices*/
  //  document.execCommand("copy");
  navigator.clipboard.writeText("otpMC_" + evt.target.innerHTML + ".gpg");
});
...
document.getElementById("pageTag").addEventListener("change", () => {
  ...
  if (
    ...
    alert(
      "Key/Box LOCK and Quick Swap can not be used at the same time\n" +
        "Please un-selected one of these options (at least temporary)",
    );
    document.getElementById("pageTag").value = localStorage.getItem(
      ":j35mc:_pageTagB4",
    );
    return;
  }
  ...
  if (document.getElementById("swapPageClear").checked != true) {
    if (
      ...
      let sure2swap = window.confirm(
        "Swaping page will clear Key/Data in Box, OK?",
      );
      if (!sure2swap) {
        document.getElementById("pageTag").value = localStorage.getItem(
          ":j35mc:_pageTagB4",
...
function delConfirmChange() {
  if (document.getElementById("delConfirm").checked === true) {
    alert(
      "You are enabling OneKey Delete\n" +
  ...
  if (
    document.getElementById("feed_box").value !=
      localStorage.getItem(
        ":j35mc:" + document.getElementById("pageTag").value + ":" +
          document.getElementById("localStoreKey").value,
      )
  ) {
    localStorage.setItem(
...
document.getElementById("boxUpdate").addEventListener("click", () => {
  ...
  document.getElementById("feed_box").value = localStorage.getItem(
    ":j35mc:" + localStorage.getItem(":j35mc:_boxChanged"),
  );
...
function storeValue() {
  ...
  if (someid1 != document.getElementById("localStoreKey").value) {
    if (
      !window.confirm(
        "The illegal characters in Key.\n " +
          "The Key will be reset as:" + someid1,
      )
    ) return;
    else document.getElementById("localStoreKey").value = someid1;
  }

  if (someid1.includes("_")) {
    alert("Key included '_' is NOT allowed");
    return;
  }
  if (someid1.includes(":")) {
    alert("Key included ':' is NOT allowed");
    return;
  }
  ...
  if (document.getElementById("pageTag").value === "*") {
    alert(
      "Unknown Page Name is NOT allow to append \n" +
        "Please change to corresponding page for update/append",
    );
    return;
  }
  ...
  if (document.getElementById("pageTag").value === "(new)") {
    ...
    if (newPage.includes("_")) {
      alert("Page Name included '_' is NOT allowed");
      return;
    }
    if (newPage.includes(":")) {
      alert("Page Name included ':' is NOT allowed");
      return;
    }
    if (newPage.includes("*")) {
      alert("Page Name included '*' is NOT allowed");
  ...
  if (someid1 != "") {
    ...
    if (localStorage.getItem(someid1) != null) { // Old data found
      if (
        localStorage.getItem(someid1) != // if found data mismatch feedbox
          document.getElementById("feed_box").value
      ) {
...
function storeChkSum() {
  ...
  if (
    ...
    alert(
      "The content of store has been changed, \n" +
    ...
    if (
      localStorage.getItem(":j35mc:_lastCheck").split(":")[2] !=
        localStorage.getItem(":j35mc:_nowXXH32")
    ) {
      localStorage.setItem(
        ":j35mc:_lastCheck",
...
function clearKey() {
  ...
  if (
    document.getElementById("localStoreKey").value.split(":")[0] === "_addPk"
  ) {
  ...
  if (
    document.getElementById("localStoreKey").value.split(":")[0] === "_rmPk"
  ) {
    if (document.getElementById("localStoreKey").value.split(":")[1] === "") {
      alert("Please specify the KeyID");
      return;
  ...
  if (document.getElementById("localStoreKey").value === "_help") {
    document.getElementById("feed_box").value = "_help : print this help \n" +
  ...
  if (
    document.getElementById("localStoreKey").value.split(":")[0] == "_setPara"
  ) {
  ...
  if (
    document.getElementById("localStoreKey").value.split(":")[0] == "_getPara"
  ) {
  ...
  if (
    document.getElementById("localStoreKey").value.split(":")[0] == "_width"
  ) {
...
function encBoxPub(inputTXT) {
  ...
  var encTXT = pKGnuPG_enc(
    inputTXT.value,
    localStorage.getItem(
      ":j35mc:_pubKey:" + document.getElementById("pubKey").value,
    ),
  );
...
>>>>>>> d4140e35c94743649fbef21bb789c8c41bcdaf88
